...
TF1* banana(int i, TH2D* slowfasttimediff_energy_hists, TH2D* energy_slowfasttimediff_hists, std::string dirPathplots)
{
    ...
    int slicewidth = 50, bin = 300, binmax = 1;
    ...
    slowfasttimediff_energy_hists->GetXaxis()->SetRange( 50, 200);
    energy_slowfasttimediff_hists->GetYaxis()->SetRange( 50, 200);
    ...
    gr1->GetYaxis()->SetRangeUser( 50, 250);
...
void align_offset(TTree *tree, std::string dirPathplots,TDirectory *dir_banana, TDirectory *dir_aligned, std::vector<Double_t> mean_fasttimediff)
{
    ...
    for (int i = 0; i < NumDetectors; i++) 
    {
        slowfasttimediff_hists[i] = new TH1D(Form("slowfasttimediff_hists%d", i), Form("Uncorrected time offset: L%d", i),  200, 0, 200);
        energy_hists[i] = new TH1D(Form("energy_hists%d", i), Form("Uncorrected energy: L%d", i),  2000, 0, 2000);
        slowfasttimediff_energy_hists[i] = new TH2D(Form("slowfasttimediff_energy_hists%d", i), Form("Uncorrected time offset vs energy: L%d", i),  200, 0, 200, 2000, 0, 2000);
        energy_slowfasttimediff_hists[i] = new TH2D(Form("energy_slowfasttimediff_hists%d", i), Form("Uncorrected energy vs time offset: L%d", i),  2000, 0, 2000, 200, 0, 200);
        timeOffsetCorrectedHists[i] = new TH1D(Form("timeOffsetCorrectedHists%d", i), Form("Corrected time offset: L%d", i),  400, -200, 200);
        timeEnergyOffsetCorrectedHists[i] = new TH2D(Form("timeEnergyOffsetCorrectedHists%d", i), Form("Corrected time offset vs energy: L%d", i),  400, -200, 200, 2000, 0, 2000);
    }
    ...
    for (int i = 0; i < NumDetectors; i++) 
    {
        ...
        for (uint64_t entry = 0; entry < numEntries; entry++) 
        {
            ...
            uint64_t nextEntry = entry;
             ...
             while (nextEntry < numEntries) 
             {
                ...
                break; // Stop searching after the first time difference is found
            }
        }
        ...
        slowfasttimediff_energy_hists[i]->GetXaxis()->SetRangeUser(0, 200);
        ...
        slowfasttimediff_hists[i]->GetXaxis()->SetRangeUser(0, 250);
