...
void timesort(TFile *outputFile, TTree* tree, std::string dirPath, std::string outputFileName)
{
    ...
    // Loop through the entries and fill the entryDataVec vector
    for (Long64_t entry = 0; entry < numEntries; entry++) {
        ...
        entryData.timeGlobal = timeGlobal[0];
        entryData.alignedTimeFast = alignedTimeFast[0];
        entryData.alignedEnergyFast = alignedEnergyFast[0];
        entryData.alignedTimeSlow = alignedTimeSlow[0];
        entryData.alignedEnergySlow = alignedEnergySlow[0];

        entryDataVec.push_back(entryData);
    }

    // Sort the entryDataVec based on timeGlobal in ascending order
    std::sort(entryDataVec.begin(), entryDataVec.end(), [](const EntryData& a, const EntryData& b) {
        return a.timeGlobal < b.timeGlobal;
    });
    ...
    for (const EntryData& entryData : entryDataVec) {
        ...
        timeGlobal[0] = entryData.timeGlobal;
        alignedTimeFast[0] = entryData.alignedTimeFast;
        alignedEnergyFast[0] = entryData.alignedEnergyFast;
        alignedTimeSlow[0] = entryData.alignedTimeSlow;
        alignedEnergySlow[0] = entryData.alignedEnergySlow;
