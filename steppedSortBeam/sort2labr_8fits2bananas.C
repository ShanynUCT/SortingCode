...
std::vector<TH1D*> slowfasttimediff_hists11(NumDetectors);
std::vector<TH1D*> timeOffsetCorrectedHistsNext(NumDetectors);
std::vector<TH1D*> timeOffsetCorrectedHistsSame(NumDetectors);
...
std::vector<TH2D*> timeEnergyOffsetCorrectedHistsSame(NumDetectors);
...
{
    // This is a Leading Edge region 
    std::cout <<"Fitting banana8 (500-2000 keV) for detector Leading Edge \n" << i << std::endl;
    ...
    gr8->GetXaxis()->SetRangeUser(500, 2000);
    TF1* fit8 = new TF1("fit8", "exp([p0]+[p1]*x)+exp([p2]+[p3]*x)", 500, 2000 ); 
    fit8->SetParameters(5.88630e+00, -1.75282e-05, 4.25855e+00, -2.19688e-03);
    gr8->Draw("AL*");
    gr8->Fit("fit8", "");
    fit8->Draw("same");
...
{
    // This is a Leading Edge region 
    std::cout <<"\nFitting banana9 (0-4000 keV) for detector Leading Edge \n" << i << std::endl;
    ...
    gr9->GetYaxis()->SetRangeUser(60, 200);
    gr9->GetXaxis()->SetRangeUser(0, 6000);
    TF1* fit9 = new TF1("fit9", "exp([p0]+[p1]*x)+exp([p2]+[p3]*x)", 50, 3250 );
    fit9->SetParameters(4.47290e+00, -2.91688e-05, 4.40237e+00, -1.65880e-03);
    gr9->Draw("AL*");
    gr9->Fit("fit9", "");
    fit9->Draw("same");
...
{
    // This is a Leading Edge region 
    std::cout <<"\nFitting banana10 (1000-6000 keV) for detector Leading Edge \n" << i << std::endl;
    ...
    gr10->GetYaxis()->SetRangeUser(0, 200);
    gr10->GetXaxis()->SetRangeUser(0, 6000);
    TF1* fit10 = new TF1("fit10", "exp([p0]+[p1]*x)+exp([p2]+[p3]*x)+exp([p4]+[p5]*x)+exp([p6]+[p7]*x)", 1000, 6000 );
    fit10->SetParameters(4.26584e+00, -9.78059e-05, -7.61846e-01, 6.13354e-04, -6.97569e+01, 1.24081e-02, -1.33085e+01, -6.78483e-03);
    gr10->Draw("AL*");
    gr10->Fit("fit10", "");
    fit10->Draw("same");
...
void process_entries_range(TFile *outputFile, std::string dirPathplots, TTree* copyTree, TDirectory *dir_banana, TDirectory *dir_aligned, unsigned long long threadStart, unsigned long long threadEnd) 
{
    ...
    correctedcorrectedTree->Branch("timeRF", &timeFastCp[RFIndex], "timeRF/D");
    correctedcorrectedTree->Branch("fastEPOLARIS", &fastEnergyPOLARISCp[0], "fastEPOLARIS/D");
    correctedcorrectedTree->Branch("fastTPOLARIS", &fastTimePOLARISCp[0], "fastTPOLARIS/D");
    ...
    correctedcorrectedcorrectedTree->Branch("timeRF", &timeFastCp[RFIndex], "timeRF/D");
    correctedcorrectedcorrectedTree->Branch("fastEPOLARIS", &fastEnergyPOLARISCp[0], "fastEPOLARIS/D");
    correctedcorrectedcorrectedTree->Branch("fastTPOLARIS", &fastTimePOLARISCp[0], "fastTPOLARIS/D");
    ...
    for (int i = 0; i < 1; i++) 
    {
        ...
        for (unsigned long long entry = threadStart; entry < threadEnd; entry++) 
        {
            ...
            for (unsigned long long nextEntry = entry + 1; nextEntry < threadEnd; nextEntry++) 
            {
                ...
                else
                {
                    ...
                    energy_slowfasttimediff_hists8[i]->Fill(energy, timeOffsetBeforeCorrection[i]);
                    entry = nextEntry;
                    break;
                }
        ...
        // ________________________________________________________________
        printf("\n\nCalculating time offset (again) for detector %d...\n", i);
        for (unsigned long long entry = threadStart; entry < threadEnd; entry++) 
        {
            ...
            for (unsigned long long nextEntry = entry + 1; nextEntry < threadEnd; nextEntry++) 
            {
                ...
                else
                {
                    ...
                    // printf("Detector %d: Time Offset Forward = %f\n", i, timeOffsetBeforeCorrectionCp[i]);

                    slowfasttimediff_hists11[i]->Fill(deltaTime);
        ...
        TCanvas* c3 = new TCanvas(Form("c3_%d", i), Form("Uncorrected time offset no2: L%d", i), 800, 600);
        slowfasttimediff_hists11[i]->GetXaxis()->SetRangeUser(-400, 400);
        slowfasttimediff_hists11[i]->SetFillColor(kBlue);
        slowfasttimediff_hists11[i]->GetXaxis()->SetTitle("Time Slow - Time Fast (ns)");
        slowfasttimediff_hists11[i]->GetYaxis()->SetTitle("Counts/ns");
        slowfasttimediff_hists11[i]->Draw();
        c3->SaveAs(Form("%s/time_offset_hist_again_L%d.root", dirPathplots.c_str(), i));
        c3->Write(Form("time_offset_hist_again_L%d", i));
        delete c3;

        TCanvas* c4 = new TCanvas(Form("c4_%d", i), Form("Uncorrected time offset no2: L%d", i), 800, 600);
        slowfasttimediff_energy_hists11[i]->GetXaxis()->SetRangeUser(-400, 400);
        slowfasttimediff_energy_hists11[i]->SetFillColor(kBlue);
        slowfasttimediff_energy_hists11[i]->GetXaxis()->SetTitle("Time Slow - Time Fast (ns)");
        slowfasttimediff_energy_hists11[i]->GetYaxis()->SetTitle("Energy (keV)");
        slowfasttimediff_energy_hists11[i]->Draw("COLZ");
        c4->SaveAs(Form("%s/time_offset_energy_hist_again_L%d.root", dirPathplots.c_str(), i));
        c4->Write(Form("time_offset_energy_hist_again_L%d", i));
        delete c4;
        ...
        energy_slowfasttimediff_hists11[i]->Draw();
        ...
        leg2->Draw();
        c8->SaveAs(Form("%s/bananas2_2Dfitfunction_L%d.root", dirPathplots.c_str(), i));
        c8->Write(Form("bananas2_2Dfitfunction_L%d", i));
        delete c8;
    
        dir_aligned->cd();

        // ________________________________________________________________
        printf("\n\nCalculating precomputed values (again) for detector %d...\n", i);
        for (Double_t e = 0; e <= 5000; e += energyStep)
        {
            if (e < 4000)
            {
        ...
        for (unsigned long long entry = threadStart; entry < threadEnd; entry++) 
        {
            ...
            // Calculate timeSlowCorrected and fill correctedTree using timeOffset
            timeSlowCorrectedCp[i] = timeSlowCorrected[i] - timeOffset * 10;
            // printf("time slow corrected = %f, time offset = %f\n", timeSlowCorrectedCp[i], timeOffset);
            correctedcorrectedcorrectedTree->Fill();
        }
        delete correctedcorrectedTree;

        // ________________________________________________________________
        printf("\n\nCalculating corrected time a final time for to plot hist for detector %d...\n", i);
        for (unsigned long long entry = threadStart; entry < threadEnd; entry++) 
        {
            ...
            if (timeSlowCorrectedCp[i] == 0) continue;
            energy = energySlowCp[i];
            ...
            {
                timeEnergyOffsetCorrectedHistsSame[i]->Fill(deltaTime, energy);
                timeOffsetCorrectedHistsSame[i]->Fill(deltaTime);
            }
        }
        for (unsigned long long entry = threadStart; entry < threadEnd; entry++) 
        {
        ...
        timeOffsetCorrectedHistsSame[i]->Write();
        timeEnergyOffsetCorrectedHistsSame[i]->Write();
        ...
        c5->SaveAs(Form("%s/time_offset_hist_corrected_next_L%d.root", dirPathplots.c_str(), i));
        delete c5;
        ...
        timeEnergyOffsetCorrectedHistsNext[i]->SetFillColor(kBlue);
        ...
        c6->SaveAs(Form("%s/time_energy_offset_hist_corrected_next_L%d.root", dirPathplots.c_str(), i));
        delete c6;

        TCanvas * c7 = new TCanvas("c7", "CorrectedTimeOffsetSame", 800, 600);
        timeOffsetCorrectedHistsSame[i]->GetXaxis()->SetRangeUser(-400, 400);
        timeOffsetCorrectedHistsSame[i]->SetFillColor(kBlue);
        timeOffsetCorrectedHistsSame[i]->GetXaxis()->SetTitle("Time Slow - Time Fast (ns)");
        timeOffsetCorrectedHistsSame[i]->GetYaxis()->SetTitle("Counts/ns");
        timeOffsetCorrectedHistsSame[i]->Draw();
        c7->SaveAs(Form("%s/time_offset_hist_corrected_same_L%d.root", dirPathplots.c_str(), i));
        delete c7;

        TCanvas * c9 = new TCanvas("c9", "CorrectedTimeOffsetEnergySame", 800, 600);
        timeEnergyOffsetCorrectedHistsSame[i]->GetXaxis()->SetRangeUser(-400, 400);
        timeEnergyOffsetCorrectedHistsSame[i]->GetYaxis()->SetRangeUser(0, 8000);
        timeEnergyOffsetCorrectedHistsSame[i]->SetFillColor(kBlue);
        timeEnergyOffsetCorrectedHistsSame[i]->GetXaxis()->SetTitle("Time Slow - Time Fast (ns)");
        timeEnergyOffsetCorrectedHistsSame[i]->GetYaxis()->SetTitle("Energy (keV)");
        timeEnergyOffsetCorrectedHistsSame[i]->Draw("colz");
        c9->SaveAs(Form("%s/time_energy_offset_hist_corrected_same_L%d.root", dirPathplots.c_str(), i));
        delete c9;
        ...
        slowfasttimediff_hists11[i]->Write();
        slowfasttimediff_energy_hists[i]->Write();
        energy_slowfasttimediff_hists[i]->Write();
        slowfasttimediff_energy_hists11[i]->Write();
        energy_slowfasttimediff_hists11[i]->Write();
    }
...
void align_offset(TFile *outputFile, std::string dirPathplots, TTree* tree, TDirectory *dir_banana, TDirectory *dir_aligned, std::vector<Double_t> mean_fasttimediff, bool& tensOfNanoSeconds, bool& hundredsOfNanoSeconds, unsigned long long numEntries) {
    ...
    for (int i = 0; i < 1; i++) 
    {
        ...
        slowfasttimediff_hists11[i] = new TH1D(Form("slowfasttimediff_hists11_%d", i), Form("Uncorrected time offset no.2: L%d", i),  600, 0, 600);
        ...
        // Full no.2
        slowfasttimediff_energy_hists11[i] = new TH2D(Form("slowfasttimediff_energy_hists11_%d", i), Form("Uncorrected time offset vs energy no.2: L%d", i),  800, -400, 400, 8000, 0, 8000);
        energy_slowfasttimediff_hists11[i] = new TH2D(Form("energy_slowfasttimediff_hists11_%d", i), Form("Uncorrected energy vs time offset no.2: L%d", i),  8000, 0, 8000, 800, -400, 400);
        ...
        // banana 8
        slowfasttimediff_energy_hists8[i] = new TH2D(Form("slowfasttimediff_energy_hists8_%d", i), Form("Uncorrected time offset vs energy: L%d", i),  280, 320, 600, 1500, 500, 2000);
        energy_slowfasttimediff_hists8[i] = new TH2D(Form("energy_slowfasttimediff_hists8_%d", i), Form("Uncorrected energy vs time offset: L%d", i),  1500, 500, 2000, 280, 320, 600);

        // banana 9
        slowfasttimediff_energy_hists9[i] = new TH2D(Form("slowfasttimediff_energy_hists9_%d", i), Form("Uncorrected time offset vs energy: L%d", i),  230, 70, 300, 4000, 0, 4000);
        energy_slowfasttimediff_hists9[i] = new TH2D(Form("energy_slowfasttimediff_hists9_%d", i), Form("Uncorrected energy vs time offset: L%d", i),  4000, 0, 4000, 230, 70, 300);

        // banana 10
        slowfasttimediff_energy_hists10[i] = new TH2D(Form("slowfasttimediff_energy_hists10_%d", i), Form("Uncorrected time offset vs energy: L%d", i),  20, 50, 70, 5000, 1000, 6000);
        energy_slowfasttimediff_hists10[i] = new TH2D(Form("energy_slowfasttimediff_hists10_%d", i), Form("Uncorrected energy vs time offset: L%d", i),  5000, 1000, 6000, 20, 50, 70);
        ...
        timeOffsetCorrectedHistsSame[i] = new TH1D(Form("timeOffsetCorrectedHistsSame%d", i), Form("Corrected time offset - Same Event: L%d", i),  800, -400, 400);
        ...
        timeEnergyOffsetCorrectedHistsSame[i] = new TH2D(Form("timeEnergyOffsetCorrectedHistsSame%d", i), Form("Corrected time offset vs energy - Same Event: L%d", i),  800, -400, 400, 8000, 0, 8000);
