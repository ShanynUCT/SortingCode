...
void timesort(TFile *outputFile, TTree* tree, std::string dirPath, std::string outputFileName)
{
    ...
    // Create a vector of tuples to store the data entries
    std::deque<std::tuple<Int_t, Double_t, Double_t, Double_t, Double_t, Double_t>> dataEntries;

    // Loop over the tree entries and store the data in the vector of tuples
    for (Long64_t entry = 0; entry < tree->GetEntries(); ++entry) 
    {
        tree->GetEntry(entry);
        dataEntries.emplace_back(detectorID, timeGlobal, alignedTimeFast, alignedEnergyFast, alignedTimeSlow, alignedEnergySlow);
    }

    // Sort the vector of tuples based on the second element (globalTime)
    std::sort(dataEntries.begin(), dataEntries.end(), [](const auto& a, const auto& b) 
    {
    return std::get<1>(a) < std::get<1>(b); // Sort based on the second element (globalTime)
    });

    // Loop over the sorted vector of tuples and fill the sortedTree
    for (const auto& entry : dataEntries) 
    {
        std::tie(sortedDetectorID, sortedGlobalTime, sortedTimeF, sortedEnergyF, sortedTimeS, sortedEnergyS) = entry;
        sortedTree->Fill();
    }
    ...
    // calculate the run duration again but this time using the firstTime as the first sortedglobalTime > 1e14
    double runDuration = 0;
    for (Long64_t entry = 0; entry < sortedTree->GetEntries(); ++entry) 
    {
        sortedTree->GetEntry(entry);
        if (sortedGlobalTime > 1e14) {
            runDuration = (lastTime - sortedGlobalTime)/60.0;
            break;
        }
    }
    printf("Run duration >1e14 startTime: %.2f min \n", runDuration);
}

void RFreduction(TTree* tree, std::string dirPath, std::string outputFileName)
{
    printf("\n Reducing RF data...\n");
