      ...
      for (i=6; i< SIZE; i+=2)
	{
	  ...
	  if ((data & 0xc0000000) == 0xc0000000)
	      ...
	      if (TSdiff > 5)
		{
		  ...
		  // Our Slow Signals are 73, 75
	      if ((ident ==  73) || (ident == 75)) // i.e. channels 9 and 11
		{
		  detectorID = ident-71; // i.e. detector2 and detector4
		  ...
		  //Our Fast Signals are 65, 67
	      if ((ident ==  65) || (ident == 67)) // i.e. channels 1 and 3
		{
		  detectorID = ident-63; // i.e. detector2 and detector4
		  energy = (a[detectorID]*adcdata*adcdata*adcdata)+(b[detectorID]*adcdata*adcdata)+(c[detectorID]*adcdata)+d[detectorID];
	      ...
	      //Ident 80,81,82,83,84,85,86,87 are timing signals of slow signals (not used)
			...
			// Timing signals of fast signals for us are channel 1+80 and 3+80
	      if ((ident ==  81) || (ident == 83))
		{
		  detectorID = ident-79;
