...
int main (int argc, char** argv)
{
...

  {
...

    {
      ...
      // Added by Steve - 07 June 2022
      //printf(" --- Inside File Loop / Index: %d --- \n", loop_count);
      ...
      // Added by Steve - 07 June 2022
      if (loop_count > 444) goto end;  // kill loop after N cycles
      ...
      for (i=6; i< SIZE; i+=2)
      {
        ...
        // Added by Steve - 07 June 2022
        //printf(" --- Inside Buffer Loop / Index: %d --- \n", buffer_count);  // resets at 8161

        data = buffer[i+1];
        TSbot = buffer[i];

        // Added by Steve - 07 June 2022
        //printf(" --- i = %d / data: %ld / TSbot: %ld --- \n", i, data, TSbot);  // resets at 8161
        ...
        if ((data & 0xc0000000) == 0xc0000000)
        {
          ...
          // Added by Steve - 07 June 2022
          //printf(" === i = %d / ident: %d / adcdata: %d / card: %d --- \n", i, ident, adcdata, card);  // resets at 8161
          ...
          if (counter == 0)
          {
            ...
            // Added by Steve - 07 June 2022
            printf(" +++ i = %d / counter: %ld / TSfirst: %ld --- \n", i, counter, TSfirst);  // resets at 8161

          }
          TSdiff = TS-TSlast;

          // Added by Steve - 07 June 2022
          //printf(" +++ i = %d / TS: %ld / TSdiff: %ld --- \n", i, TS, TSdiff);  // resets at 8161
          ...
          // TIMING DETAILS for 220608 runs 4 & 5
          // -> from Shanyn
          // Fast 1: 72           Fast 2: 73
          // Slow 1: 64           Slow 2: 65
          // Fast 1 timing: 88    Fast 2 timing: 89
          // Slow 1 timing: 80    Slow 2 timing: 81
          // Polaris: 79          Polaris timing (?): 95

          // Added by Steve - 07 June 2022
          if (ident > 89 && ident < 100) {
            //printf(" === i = %d / ident: %d / energy: %d / TS: %ld --- \n", i, ident, energy, TS);  // resets at 8161
          }

          if (ident == 79) {
            time_diff = TS - time_last;
            printf(" === i = %d / ident: %d / energy: %d / TS: %ld / TD: %ld --- \n", i, ident, energy, TS, time_diff);
            time_last = TS;
          }
...
// generate a uniform random number 
  ...
  rnd->SetSeed(0);
  double rnd_num = rnd->Uniform(0,1);
