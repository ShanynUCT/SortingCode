...
/* COMPILE: g++ -std=c++0x labrsort5_v3.C -o exe `root-config --cflags --libs` -lSpectrum */
...
int main (int argc, char** argv)
{
  ...
  LaBrData->Branch("insyncEL0", &energyS[0], "insyncEL0/D");
  ...
  LaBrData->Branch("insyncEL1", &energyS[1], "insyncEL1/D");
  ...
  LaBrData->Branch("insyncEL2", &energyS[2], "insyncEL2/D");
  ...
  LaBrData->Branch("insyncEL3", &energyS[3], "insyncEL3/D");
      ...
      for (i=6; i< SIZE; i+=2)
      {
        ...
        if ((data & 0xc0000000) == 0xc0000000)
        {
           ...
           */
            ...
            //std::cout << "Event " << item << " -> " << ident << " : " << TS << " " << TSdiff << " " << adcdata << std::endl;
        ...
        if (TSdiff > Window)
        {
          ...
          { 
            ...
            {
              //Singles spectra       
              //std::cout << "L" << j <<  "| timeRF " << timeF[4] << "| energyS " << energyS[j] << "| timeF " << timeF[j] << "| timeS " << timeS[j] << "| TD_rfS " << timeF[4]/10-timeS[j]/10 << "| TS_rfF " << timeF[4]/10-timeF[j]/10<< std::endl;
              slowE[j]->Fill(energyS[j]);
              uncalibE[j]->Fill(energySUncalib[j]);
              hit1->Fill(j);
              totalcount[j]++;
                ...
                if (j != k )
                {
                  ...
                  if (j == 4) 
                  {
                    ...
                    if ((tdF >= 212) && (tdF <= 312)) 
                    {
                      ...
                      insynccount[k]++;
                      //if ((k == 1)) {std::cout << "LaBrData" << k << " " << "| EnergyS:" << energyS[k] << " "<< "| TimeF:" << timeF[k] << " " << "| TimeS:"<< timeS[k] << std::endl;}
                    }
                  }
  ...
  // Show  what the first 1000 entries in LaBrData tree look like
  // LaBrData -> Scan("*", "", "", 200, 0);
  //LaBrData -> Show();
  time(&end);
